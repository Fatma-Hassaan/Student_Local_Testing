import numpy as np
import pandas as pd

def document_hyperparameter_tuning_clamp(train_df: pd.DataFrame, test_df: pd.DataFrame) -> dict:
    # TODO: Read the function description in https://github.gatech.edu/pages/cs6035-tools/cs6035-tools.github.io/Projects/Machine_Learning/Task5.html and implement the function as described
    hyperparameters = {}
    return hyperparameters

def train_model_return_scores_clamp(train_df: pd.DataFrame, test_df: pd.DataFrame) -> pd.DataFrame:
    # TODO: Read the function description in https://github.gatech.edu/pages/cs6035-tools/cs6035-tools.github.io/Projects/Machine_Learning/Task5.html and implement the function as described
    test_scores = pd.DataFrame()
    return test_scores 

def document_hyperparameter_tuning_unsw(train_df: pd.DataFrame, test_df: pd.DataFrame) -> dict:
    # TODO: Read the function description in https://github.gatech.edu/pages/cs6035-tools/cs6035-tools.github.io/Projects/Machine_Learning/Task5.html and implement the function as described
    hyperparameters = {}
    return hyperparameters

def train_model_return_scores_unsw(train_df: pd.DataFrame, test_df: pd.DataFrame) -> pd.DataFrame:
    # TODO: Read the function description in https://github.gatech.edu/pages/cs6035-tools/cs6035-tools.github.io/Projects/Machine_Learning/Task5.html and implement the function as described
    test_scores = pd.DataFrame()
    return test_scores 
