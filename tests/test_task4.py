...
#from task4 import calculate_naive_metrics
class Test_calculate_naive_metrics(unittest.TestCase):
    def setUp(self):
        ...
        # Inputs
        self.train_features = pd.read_csv(os.path.join(folder_path4,"NATICUSdroid_train_features.csv"), index_col=0)
        self.test_features  = pd.read_csv(os.path.join(folder_path4,"NATICUSdroid_test_features.csv"), index_col=0)
        self.train_targets  = pd.read_csv(os.path.join(folder_path4,"NATICUSdroid_train_targets.csv"), index_col=0)["Result"]
        self.test_targets   = pd.read_csv(os.path.join(folder_path4,"NATICUSdroid_test_targets.csv"), index_col=0)["Result"]
...
#from task4 import calculate_logistic_regression_metrics
class Test_calculate_logistic_regression_metrics(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            self.importance_df_ans = pd.read_pickle(os.path.join(pkl_files_folder,"logreg_importance.pkl"))
        except:
            print("Missing answer pkl files")
    ...
    def setUp(self):
        ...
        try:
            with open(os.path.join(pkl_files_folder,"dt_metrics.pkl"), 'rb') as file:
                self.dt_metrics_ans = pickle.load(file)
            self.importance_df_ans = pd.read_pickle(os.path.join(pkl_files_folder,"dt_importance.pkl"))
        except:
            print("Missing answer pkl files")

        self.dt_kwargs = {'criterion': 'entropy', 'max_depth': 3, 'random_state': 0}

        # Calculate Metrics with Student's Function
        self.dt_metrics, _ = calculate_decision_tree_metrics(
            self.train_features,
            self.test_features,
            self.train_targets,
            self.test_targets,
...
#from task4 import calculate_random_forest_metrics
class Test_calculate_random_forest_metrics(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            self.importance_df_ans = pd.read_pickle(os.path.join(pkl_files_folder,"rf_importance.pkl"))
        except:
            print("Missing answer pkl files")
